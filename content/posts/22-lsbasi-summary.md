...
2. 如果看不明白文章写的内容的话，输入例子，调试一遍，知晓每一步的流程能解决 80% 的问题。
3. Part 7 和 Part 14 是两个难度跃迁的章节，其他章节都很平滑。
4. 如果看不明白或者掌控不住可以从头开始。这不会耽误很多时间，恰恰相反就能搞定。
...
6. 开始的时候语法树很简单，但是后续就很复杂了，可以结合着调试器来分析。一定要结合着图来看，每一步走到哪里，不然太抽象。也就是解释器代码（Python），语法树图片，被解释的源码（Pascal）三者之间的映射要理解到位。
...
这个项目作为入门编译原理是一个很好的开始。收获很多，目前还有很多工作，继续做下去意义不大了。等作者更新了在继续 coding 。

这篇文章后续一定会重写完善，这个项目很有趣，代码我会重构的。
